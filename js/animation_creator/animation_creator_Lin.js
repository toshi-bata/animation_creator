...
animation_creator.prototype.start = function (viewerMode, modelName, reverseProxy, extension) {
    var _this = this;
...
animation_creator.prototype._initEvents = function () {
    var _this = this;
    ...
    function resetCommand() {
        ...
        if (_this._previewArrow != undefined) {
            _this._viewer.getMarkupManager().unregisterMarkup(_this._previewArrow);
...
//                nodes = _this._handleOp.getNodes().concat();
//                _this._handleOp.reset();
//                resetOperator();
            ...
            switch (command) {
                case "home":
                    _this._homeCamera = _this._viewer.getView().getCamera();
...
//                    OM.push(_this._handleOpHandel);
                ...
                case "hide":
                    if (nodes.length == 0) {
                        var selections = _this._viewer.getSelectionManager().getResults();
                        if (selections.length == 0)
                            return;
                        for (var i = 0; i < selections.length; i++) {
                            var selItem = selections[i];
                            nodes.push(_this._viewer.model.getNodeParent(selItem.getNodeId()));
                        }
                    }
    ...
    var url = "../common/jQuery-File-Upload/php/up.php";
    ...
    $("#dlOptionsDlg").dialog({
        ...
        buttons: {
            'OK': function(){
                var options = "000";
                options = options + ($("#dlOp_anim").prop('checked') ? '1': '0');
                options = options + ($("#dlOp_cube").prop('checked') ? '1': '0');
                options = options + ($("#dlOp_triad").prop('checked') ? '1': '0');
                options = options + ($("#dlOp_toolbar").prop('checked') ? '1': '0');
                options = options + ($("#dlOp_std").prop('checked') ? '1': '0');
                
                $(this).dialog('close');
                $('#loadingImage').show();
                _this._animationSteps.update();
                _this._animationSteps.createScript().then(function(){
                    return new Promise(function (resolve, reject) {
                        var cmd ='./ExportToHtml "../data/' + _this._modelName + '/' + _this._modelName + '.' + _this._extension + '" "template/htmltemplate.html" "html" "../html/' + _this._modelName + '.html" "../html/log.txt" ' + options;

                        $.ajax({
                            url: "../common/php/cmdExec_Lin.php",
                            type: "post",
                            dataType: 'json',
                            data: {
                                exeDir: '../../animation_creator/createHtml/',
                                exePath: cmd
                            }
                        }).done(function (response) {
                            var xhr = new XMLHttpRequest(),
                                  a = document.createElement('a'), file;
                            xhr.open('GET', "html/" + _this._modelName + ".html", true);
                            xhr.responseType = 'blob';
                            xhr.onload = function () {
                                file = new Blob([this.response], { type : 'application/octet-stream' });
                                a.href = window.URL.createObjectURL(file);
                                a.download = _this._modelName + ".html";
                                a.click();
                            };
                            xhr.send();
                            $('#loadingImage').hide();
...
animation_creator.prototype._createViewer = function (viewerMode, modelName, reverseProxy) {
    var _this = this;
    createViewer(viewerMode, modelName, "container", reverseProxy).then(function (hwv) {
        ...
        _this._animationSteps = new AnimationSteps(_this._resource.getMsgs(), _this._resource.getTypes());
...
animation_creator.prototype.highlightNode = function(nodeId, key_ctrl) {
    var _this = this;
...
animation_creator.prototype.resetLineHighlighted = function () {
    var _this = this;
...
animation_creator.prototype.setVector = function(vector) {
    var _this = this;
...
animation_creator.prototype.showPreview = function(vector) {
    var _this = this;
    ...
    _this._viewer.model.getNodesBounding(nodes).then(function(box){
        ...
        switch (command) {
            case "translate":
                ...
                var markupItem = new ArrowMarkup(_this._viewer, p1, p2);
                _this._previewArrow = _this._viewer.getMarkupManager().registerMarkup(markupItem);
