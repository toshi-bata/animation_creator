...
export class myHandleOperator extends Communicator.Operators.HandleOperator {
    ...
    setPoint(position) {
        ...
        this.addTrackedPoint(position);
        this._stPnt = position;

        // Start group
        this._owner._animationSteps.startGroup();
    }

    removeHandles() {
        super.removeHandles();
        this.clearTrackedPoints();

        // Stop group
        this._owner._animationSteps.stopGroup();
    }

    onMouseUp(event) {
        super.onMouseUp(event);

        const points = this.getTrackedPoints();
        if (points.length) {
            // Quit if tracked point isn't moved
            if (this._stPnt.equals(points[0])) {
                return;
            }
            
            const nodeIds = super.getNodeIds(0);

            // Create step line
            this._owner._handleOpOp.createStepLine(this._stPnt, points[0].copy());

            // Create animation step
            const axis = Communicator.Point3.subtract(points[0], this._stPnt);
            axis.normalize();
            const distance = Communicator.Point3.distance(points[0], this._stPnt);
            const start = this._owner._animationSteps.getTerminationTime();
            this._owner._animationSteps.addTranslationStep(nodeIds, axis, distance, start, 1, [this._stPnt]);

            // Set end point as next start point
            this._stPnt = points[0];
            this._groupMemberCnt = 0;
        }
        else {
...
export class HandleOperatorOperator {
    constructor(viewer, handleOp, handleOpHandle) {
        ...
        // }
        // else {
        //     viewer.operatorManager.push(handleOpHandle)
        // }
    ...
    addHandle(nodeId) {
        // Show handle
        this._myHandleOp.addHandles([nodeId], this._centerPnt, 0).then(() => {
            this._myHandleOp.setPoint(this._centerPnt);
            ...
            this._myHandleOp.updatePosition(new Communicator.Point3(0, 0, 0), rotation, true, 0);
    ...
    setHandleNodeId(nodeId) {
        this._myHandleOp.setNodeIds([nodeId], 0);
    }
    ...
    rewind() {
        ...
        this._myHandleOp.removeHandles();
